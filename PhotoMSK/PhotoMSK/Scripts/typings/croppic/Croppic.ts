module CroppicModule {

    export class CroppicOptions {
        uploadUrl = '';
        uploadData = {};
        cropUrl = '';
        cropData = {};
        ...
        //callbacks
        onBeforeImgUpload: any;
        onAfterImgUpload: any;
        onImgDrag: any;
        onImgZoom: any;
        onImgRotate: any;
        onBeforeImgCrop: any;
        onAfterImgCrop: any;
        onBeforeRemoveCroppedImg: any;
        onAfterRemoveCroppedImg: any;
        onError: any;

    }

    export class Croppic {
        id = '';
        options: CroppicOptions;
        imgInitW = 0;
        imgInitH = 0;
        imgW = 0;
        imgH = 0;
        ...
        iframeform: JQuery;
        iframeobj: JQuery;
        cropControlsUpload: JQuery;
        cropControlsCrop: JQuery;
        cropControlZoomMuchIn: JQuery;
        cropControlZoomMuchOut: JQuery;
        cropControlZoomIn: JQuery;
        cropControlZoomOut: JQuery;
        cropControlCrop: JQuery;
        cropControlReset: JQuery;
        cropControlRemoveCroppedImage: JQuery;
        modal: JQuery;
        loader: JQuery;
        imgUrl: string;
        ...
        bindImgUploadControl() {
            ...
            that.form.find('input[type="file"]').change(function () {
                ...
                if (that.options.processInline) {
                    ...
                    var files = <any> that.form.find('input[type="file"]')[0];
                    reader.readAsDataURL(files.files[0]);

                } else {

                    var formData = new window['FormData'](that.form[0]);
                    ...
                    $.ajax(<JQueryAjaxSettings> ajaxsettings).always(data => {
        ...
        afterUpload(data) {
            ...
            if (response.status == 'success') {
                ...
                img.load(() => {
                    this.initCropper();
                    this.hideLoader();
                    if (this.options.onAfterImgUpload) this.options.onAfterImgUpload.call(this);
                });
        ...
        initialScaleImg() {
            ...
            if (that.options.enableMousescroll) {
                that.img.on('mousewheel', (event: any) => {
                    event.preventDefault();
                    this.zoom(this.options.zoomFactor * event.deltaY);
                });
            }
        ...
        createCropControls() {
            ...
            if (that.options.doubleZoomControls) {
                ...
                that.cropControlZoomMuchIn.on('click', () => { this.zoom(this.options.zoomFactor * 10); });

                that.cropControlZoomMuchOut = that.cropControlsCrop.find('.cropControlZoomMuchOut');
                that.cropControlZoomMuchOut.on('click', () => { this.zoom(-this.options.zoomFactor * 10); });
            }

            that.cropControlZoomIn = that.cropControlsCrop.find('.cropControlZoomIn');
            that.cropControlZoomIn.on('click', () => { this.zoom(this.options.zoomFactor); });
        ...
        initDrag() {
            ...
            that.img.on("mousedown touchstart", function (e: any) {
                ...
                that.img.css('z-index', 1000).on("mousemove touchmove", (e: any) => {
        ...
        crop() {
            ...
            if (typeof FormData == 'undefined') {
                ...
                XHR.setRequestHeader('Content-Length', String(urlEncodedData.length));
        ...
        createFallbackIframe() {
            ...
            if (!that.isAjaxUploadSupported()) {
                ...
                var elem = <any> that.iframeform.find("input")[0];
                elem.attachEvent("onchange", () => {
                    this.submitFallbackIframe(this);
                });

                var eventHandlermyFile = () => {
                    if (iframe.detachEvent)
                        iframe.detachEvent("onload", eventHandlermyFile);
                    else
                        iframe.removeEventListener("load", eventHandlermyFile, false);

                    var response = this.getIframeContentJSON(iframe);

                    if (jQuery.isEmptyObject(this.modal)) {
