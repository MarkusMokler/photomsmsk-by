...
function smartProperty(obj, name) { // get a camel-cased/namespaced property of an object
    ...
    function htmlEscape(s) {
        return s.replace(/&/g, '&amp;')
            ...
            .replace(/"/g, '&quot;')
            .replace(/\n/g, '<br />')
            .replace("%", "<span class='calendar-precent'>")
            .replace("/%", "</span>")
            .replace("#", "<span class='calendar-sharp'>")
            .replace("/#", "</span>");
    }
...
function AgendaView(element, calendar, viewName) {
	...
	function setHeight(height) {
		...
		var bodyHeight = Math.min( // total body height, including borders
            ...
            // var slotHeight0 = slotTable.find('tr:first').height() + 2; // +1 for bottom border
            slotHeight = slotTable.find('tr:eq(1)').height() + 1;
            // HACK: i forget why we do this, but i think a cross-browser issue
            //  slotHeight = slotHeight0;//(slotHeight0 + slotHeight1) / 2;
